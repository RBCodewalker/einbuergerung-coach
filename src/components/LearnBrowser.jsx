...
export function LearnBrowser({ data, hideNavigation = false, currentIndex: propCurrentIndex }) {
  const navigate = useNavigate();
  const { questionIndex } = useParams();
  const { selectedState } = useQuiz();
  const currentIndex = propCurrentIndex !== undefined ? propCurrentIndex : parseInt(questionIndex, 10) - 1; // Convert from 1-based to 0-based
